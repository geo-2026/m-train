...
(function () {
  ...
  function makeQrDataUrl(text) {
    try {
      ...
      return qr.createDataURL(6, 8); // cellSize, margin
    } catch (_) { return ''; }
  }
  function showAccess() {
    const box = $('#accessBox');
    ...
    const img = makeQrDataUrl(url);
    box.innerHTML = `
      <div class="qr-box">
        <div class="muted">학생들은 아래 주소로 접속하거나 QR을 스캔하세요</div>
        <div class="url-pill">${esc(url)}</div>
        ${img ? `<div><img src="${img}" alt="QR" width="220" height="220"></div>` : ''}
      </div>`;
  }
  ...
  renderers[9] = function () {
    ...
    $('#b9send').addEventListener('click', async () => {
      ...
      try {
        ...
        await MTBackend.submitQuestions(S.cls, clientId(), kq, lq);
  ...
  renderers[10] = function () {
    $('#page10').innerHTML = `
      ...
      <div class="panel">
        ...
        <div class="center mt">
          <button class="btn red" id="t10clear">🧹 수업 종료 — 우리 반 질문·대화 모두 삭제</button>
          <div class="muted" style="margin-top:6px">학생 단말기의 PDF는 영향을 받지 않아요. (서버에는 익명 데이터만 임시 저장됩니다.)</div>
        </div>
      </div>`;
    $('#t10home').addEventListener('click', () => { stopPoll(); go(1); });
    $('#t10chat').addEventListener('click', () => { pendingChat = null; stopPoll(); go(11); });
    $('#t10clear').addEventListener('click', async () => {
      if (!confirm(`${S.cls}반의 저장된 질문과 대화를 모두 삭제할까요?\n(학생 단말기의 PDF는 영향을 받지 않습니다.)`)) return;
      try {
  ...
  async function sendChat() {
    ...
    if (!text) return;
    if (!teacherKey) { toast('먼저 Claude API 키를 입력해 주세요.', 'err'); return; }
    inp.value = '';
    const history = (chats[curPhil] || []).slice();   // 이번 메시지 이전까지의 대화(AI 호출용)
    chats[curPhil].push({ role: 'user', content: text });
    chats[curPhil].push({ role: 'assistant', content: '…' });
    renderChat();
    try {
      const reply = await MTai.chat({
        persona: window.MT_PERSONAS[curPhil], history, message: text,
        apiKey: teacherKey, model: S.model,
      });
      chats[curPhil][chats[curPhil].length - 1] = { role: 'assistant', content: reply };
      renderChat();
      // 학생들이 12페이지에서 볼 수 있도록 저장(익명, 학급별). 저장 실패는 대화 흐름을 막지 않음.
      try { await MTBackend.appendChat(S.cls, curPhil, text, reply); } catch (_) {}
    } catch (e) {
      chats[curPhil].pop(); // remove thinking
      chats[curPhil].pop(); // remove optimistic user msg
      renderChat();
      toast(e.message, 'err');
      inp.value = text;
    }
  ...
  async function resetChat() {
    ...
    try {
      await MTBackend.clearChat(S.cls, curPhil);
  ...
  async function saveTeacherPdf() {
    ...
    try {
      let questions = [];
      try { const st = await MTBackend.getState(S.cls); questions = st.questions || []; } catch (_) {}
      await MTpdf.teacher({
        cls: S.cls, questions,
        kantChat: chats.kant || [], leopoldChat: chats.leopold || [],
      });
  ...
  //   MTBackend.getChat 으로 우리 반의 저장된 대화를 불러와 그대로 보여줍니다.
  // ========================================================================
  ...
  // ---------- boot ----------
  window.MT_CLASSES = (window.MT_CONFIG && window.MT_CONFIG.classes) || ['1-1', '1-2', '1-3', '1-4', '1-5', '1-6'];
  async function boot() {
    load();
